...
def parse_kgat_args():
    ...
    parser.add_argument("--data_name", nargs="?", default='amazon-book', help='Choose a dataset from {yelp2018, last-fm, amazon-book}')
    
    parser.add_argument("--data_dir", nargs="?", default="datasets/", help = "Input data path")

    # 이거 되게 신기하당..
    parser.add_argument("--use_pretrain", type = int, default=1, help = "0: No pretrain, 1: Pretrain with the learned embeddings, 2: Pretrain with stored model.")
    ...
    parser.add_argument("--test_batch_size", type = int, default = 10000, help='Test batch size (the user number to test every batch).')
