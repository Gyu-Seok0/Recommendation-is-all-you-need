...
class KGAT(nn.Module):
    def __init__(self,args,
        ...
        self.conv_dim_list = [args.embed_dim] + eval(args.conv_dim_list) # [64,64,32,16]
    ...
    def calc_cf_embeddings(self):
        ego_embed = self.entity_user_embed.weight # entity_user_embed
        all_embed = [ego_embed]

        for idx, layer in enumerate(self.aggregator_layers):
            ego_embed = layer(ego_embed, self.A_in) #ego_embed가 아니라 e_h가 맞는 것 같긴한데 아무튼 이 값이 만들어질떄 A_in (attention map인듯?)이 고정되서 사용됌.
