...
class DataLoaderBase(object):
    def __init__(self,args, logging):
        ...
        self.use_pretrin = args.use_pretrain
        self.pretrain_embedding_dir = args.pretrain_embedding_dir

        self.data_dir = os.path.join(args.data_dir, args.data_name)
        self.train_file = os.path.join(self.data_dir, 'train.txt')
        self.test_file = os.path.join(self.data_dir, 'test.txt')
        self.kg_file = os.path.join(self.data_dir, "kg_final_test.txt") # 나중에 고치기
        ...
        if self.use_pretrin == 1:  # "0: No pretrain, 1: Pretrain with the learned embeddings, 2: Pretrain with stored model."
    ...
    # 기본적으로 sampling은 while문을 기반으로, numpy에서 randomly하게 index를 sampling해서 작동한다.
    ...
    def generate_kg_batch(self,kg_dict,batch_size,highest_neg_idx):
        ...
        if batch_size <= len(exist_heads):
            ...
            batch_head = [random.sample(exist_heads) for _ in range(batch_size)]
