...
class DataLoaderKGAT(DataLoaderBase):
    def __init__(self, args, logging):
        ...
        kg_data = self.load_kg(self.kg_file) # head-relation-tail로 이미 구성되어 있음. DF형태로(H,R,T)로 이뤄짐.
        self.construct_data(kg_data) 
        self.print_info(logging)

        self.laplacian_type = args.laplacian_type # "Specify the type of the adjacency(laplacian) matrix from {symmetric, random-walk}"
    ...
    def construct_data(self, kg_data):
        ...
        kg_data = pd.concat([kg_data, inverse_kg_data], axis=0, ignore_index=True, sort=False) # kg_data + inverse_kg_data
        
        # re-map user id
        kg_data['r'] += 2 # cf2kg_train_data의 관계가 0, inverse_cf2kg_train_data의 관계가 1
        self.n_relations = max(kg_data['r']) + 1
        self.n_entities = max(max(kg_data['h']), max(kg_data['t'])) + 1
        self.n_users_entities = self.n_users + self.n_entities # user + entity
        

        # user역시 entity로 포함시키기 위해서, n_entities 값을 더해서 저장함 (user-item)
    ...
    # coo tensor는 indices와 values를 가지는 tensor라고 이해가능하다.
    def convert_coo2tensor(self, coo):
        values = coo.data
        indices = np.vstack((coo.row, coo.col)) # row아래 col을 쌓음. (vertical하게, axis = 0로 concat하는 느낌)
    ...
    def create_adjacency_dict(self):
        ...
        for r, ht_list in self.train_relation_dict.items():
            ...
            self.adjacency_dict[r] = adj #각각의 관계별로 head-tail에 대한 matrix를 담아내고 있음.
    ...
    def create_laplacian_dict(self):
        ...
        def random_walk_norm_lap(adj):
            rowsum = np.array(adj.sum(axis=1)) # 각 head가 연결된 tail의 수

            d_inv = np.power(rowsum, -1.0).flatten() # 역수
            d_inv[np.isinf(d_inv)] = 0 # inf는 0으로 대체
            d_mat_inv = sp.diags(d_inv) # diagonal matrix로 바꿈.

            norm_adj = d_mat_inv.dot(adj) # 각각의 d_inv값들이 곱해진다고 생각하면됌 -> normalized됌
            return norm_adj.tocoo()
        ...
        A_in = sum(self.laplacian_dict.values()) # 각 matrix의 값들이 더해짐.
        self.A_in = self.convert_coo2tensor(A_in.tocoo()) # 95690 x 95690의 matrix
